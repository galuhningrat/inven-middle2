...
<div class="data-table-container" id="reportContainer">
    ...
    <div style="padding: 2rem;" id="printArea">
        <!-- Report Header -->
        <div class="report-header-print" style="text-align: center; margin-bottom: 2rem; border-bottom: 3px double var(--border-color); padding-bottom: 1.5rem;">
            <div style="display: flex; align-items: center; justify-content: center; gap: 1.5rem; margin-bottom: 1rem;">
                <img src="{{ asset('images/logo-stti.png') }}" alt="Logo" style="width: 80px; height: 80px; object-fit: contain;">
                <div style="text-align: left;">
                    <h1 style="margin: 0; font-size: 1.25rem; color: var(--text-primary);">SEKOLAH TINGGI TEKNOLOGI INDONESIA</h1>
                    <h2 style="margin: 0; font-size: 1rem; color: var(--text-secondary); font-weight: normal;">CIREBON</h2>
                    <p style="margin: 0.25rem 0 0; font-size: 0.75rem; color: var(--text-secondary);">Jl. Raya Cirebon - Kuningan, Desa Kondangsari</p>
                </div>
            </div>
            <h2 style="margin: 1rem 0 0.5rem; font-size: 1.1rem; text-transform: uppercase; letter-spacing: 2px;">{{ $title }}</h2>
            <p style="color: var(--text-secondary); font-size: 0.875rem;">
                Dicetak oleh: <strong>{{ $user->name }}</strong> | Tanggal: <strong>{{ now()->format('d F Y H:i') }}</strong>
            </p>
        </div>
                    ...
                    <tbody>
                        @foreach($data as $index => $borrowing)
                            <tr>
                                ...
                                <td>
                                    <span class="status-badge {{ $borrowing->status === 'Selesai' ? 'available' : ($borrowing->status === 'Terlambat' ? 'maintenance' : 'borrowed') }}">
                                        {{ $borrowing->status }}
                                    </span>
                                </td>
                            </tr>
                ...
                <table class="data-table" style="font-size: 0.8rem;">
                    ...
                    <tbody>
                        @foreach($data as $index => $maintenance)
                            <tr>
                                ...
                                <td>
                                    <span class="status-badge {{ $maintenance->status === 'Selesai' ? 'available' : 'maintenance' }}">
                                        {{ $maintenance->status }}
                                    </span>
                                </td>
                            </tr>
                    ...
                    <tfoot>
                        <tr style="background: var(--light-bg); font-weight: bold;">
                            ...
                            <td colspan="2" style="text-align: left;">Rp {{ number_format($data->sum('cost'), 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td colspan="8" style="text-align: center; font-size: 0.75rem; color: var(--text-secondary);">
        ...
        @elseif($type === 'financial')
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
                ...
                <div style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white; padding: 1.5rem; border-radius: 12px; text-align: center;">
                    ...
                    <p style="font-size: 1.25rem; font-weight: bold; margin: 0;">Rp {{ number_format($data['total_value'], 0, ',', '.') }}</p>
                    <p style="font-size: 0.75rem; opacity: 0.8; margin-top: 0.5rem;">nilai inventaris</p>
                </div>
                <div style="background: linear-gradient(135deg, #eb3349 0%, #f45c43 100%); color: white; padding: 1.5rem; border-radius: 12px; text-align: center;">
                    <h4 style="margin-bottom: 0.5rem; opacity: 0.9; font-size: 0.875rem;">Biaya Pemeliharaan</h4>
                    <p style="font-size: 1.25rem; font-weight: bold; margin: 0;">Rp {{ number_format($data['maintenance_cost'], 0, ',', '.') }}</p>
                ...
                <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 1.5rem; border-radius: 12px; text-align: center;">
                    ...
                    <p style="font-size: 1.25rem; font-weight: bold; margin: 0;">Rp {{ number_format($data['average_value'], 0, ',', '.') }}</p>
